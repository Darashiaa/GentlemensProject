	...
	<div id="site_content">	
		...
		<div id="main_wrapper">
		 ...
		 <%
                ArrayList<String> caseNames = new ArrayList<String>(); 
				 ArrayList<String> casePrice = new ArrayList<String>();
				  ArrayList<String> casePhoto = new ArrayList<String>();
				  ArrayList<String> caseDescription = new ArrayList<String>();
				  ArrayList<String> caseLink = new ArrayList<String>();
				  ArrayList<String> caseFormFactor = new ArrayList<String>();
			
            try {
                Connection con = (Connection) DriverManager.getConnection("jdbc:neo4j://localhost:7474/");
                String query = ("Match (n:`Case`) return n");
            ...
            try(ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					...
					String FormFactor = jso.get("Formfactor").toString();
					caseNames.add(name);
					casePrice.add(price);
					casePhoto.add(photo);
					caseLink.add(link);
					caseDescription.add(description);
					caseFormFactor.add(FormFactor);
		...
		<%	for(int i = 0; i < 500; i++)
			...
			<div id = <%= i %> class = "product_wrapper"> 	
			  <br><img src=<%=casePhoto.get(i)%> width="50%" height="50%"/></br>
			  <br><%=caseNames.get(i)%></br>
			  <br>€ <%=casePrice.get(i)%></br>
					<div id = "hover_info">
				<p><a href=<%=caseLink.get(i)%> class='linkToWebsite'>Go to website</a></p>	
					<b>Description:</b>		
				<p><%=caseDescription.get(i)%></p>
					<b>Formfactor:</b>	
				<p><%=caseFormFactor.get(i)%></p>
