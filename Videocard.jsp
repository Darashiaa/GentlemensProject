...
<html>
	...
	<div id="site_content">	
		...
		<div id="main_wrapper">
		 ...
		 <%
                ArrayList<String> VideocardNames = new ArrayList<String>(); 
				 ArrayList<String> VideocardPrice = new ArrayList<String>();
				  ArrayList<String> VideocardPhoto = new ArrayList<String>();
			
            try {
                Connection con = (Connection) DriverManager.getConnection("jdbc:neo4j://localhost:7474/");
                String query = ("Match (n:`Videocard`) return n");
            ...
            try(ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					...
					VideocardNames.add(name);
					VideocardPrice.add(price);
					VideocardPhoto.add(photo);
		...
		<%	for(int i = 0; i < 500; i++)
			...
			<div id = <%= i %> class = "product_wrapper"> 
			  <br><img src=<%=VideocardPhoto.get(i)%> width="50%" height="50%"/></br>
			  <br><%=VideocardNames.get(i)%></br>
			  <br>€ <%=VideocardPrice.get(i)%></br>
			</div>	
		<%		} %>
