 ...
 * @author UR mama
 */
public class CaseController extends ReadController{
        ...
        ArrayList <String[]> fieldsArrays = readFile(csvFile);
           for (String[] fields : fieldsArrays) {
               try {
                   ...
                   assignValuesToHashMap();
                   fill(new Case(price,name,desc,photo,formFactor,link,site, label));
    ...
    public void addRelations(Node node, Case case1) {         
        Label labelCase = DynamicLabel.label("Motherboard");
        String[] formfactors = case1.getBouwvorm().split(",");
            for(int i = 0; formfactors.length > i; i++) {
                Iterator<Node> nodes = graphdb.findNodesByLabelAndProperty(labelCase, formFactorProperty, formfactors[i]).iterator();
                while(nodes.hasNext()) {
                    Node caseNode = nodes.next();
                    node.createRelationshipTo(caseNode, Relationship.IS_COMPATIBLE_WITH);
                    caseNode.createRelationshipTo(node, Relationship.IS_COMPATIBLE_WITH);                    
                }
            }
    }
}
