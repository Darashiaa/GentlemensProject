	...
	<div id="site_content">	
		...
		<div id="main_wrapper">
		 ...
		 <%
                ArrayList<String> MemoryNames = new ArrayList<String>(); 
				 ArrayList<String> MemoryPrice = new ArrayList<String>();
				  ArrayList<String> MemoryPhoto = new ArrayList<String>();
				  ArrayList<String> MemoryDescription = new ArrayList<String>();
				  ArrayList<String> MemoryLink = new ArrayList<String>();
				  
			
            try {
                Connection con = (Connection) DriverManager.getConnection("jdbc:neo4j://localhost:7474/");
                String query = ("Match (n:`Memory`) return n");
            ...
            try(ResultSet rs = stmt.executeQuery()){
				while (rs.next()) {
					...
					MemoryPrice.add(price);
					MemoryPhoto.add(photo);
					MemoryLink.add(link);
					MemoryDescription.add(description);
		...
		<%	for(int i = 0; i < 500; i++)
			...
			<div id = <%= i %> class = "product_wrapper"> 	
			  <br><img src=<%=MemoryPhoto.get(i)%> width="50%" height="50%"/></br>
			  <br><%=MemoryNames.get(i)%></br>
			  <br>€ <%=MemoryPrice.get(i)%></br>
					<div id = "hover_info">
				<p><a href=<%=MemoryLink.get(i)%> class='linkToWebsite'>Go to website</a></p>	
					<b>Description:</b>		
				<p><%=MemoryDescription.get(i)%></p>
